    ...
    Do While index > 0 And index <= Len(Str)
        ...
        Select Case (Char)
            ...
            Case quote
                ...
                parseString = RestoreJsonSymbol(SB.ToString)
                Set SB = Nothing
...
Public Function ConvertJsonSymbol(ByVal strJSON As String) As String
    strJSON = Replace(strJSON, vbCr, "%0D", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, vbLf, "%0A", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "&", "%26", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, " ", "%20", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "'", "%39", 1, -1, vbBinaryCompare)

    strJSON = Replace(strJSON, "\", "%5c", 1, -1, vbBinaryCompare)
    
    strJSON = Replace(strJSON, """", "%22", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, ":", "%3A", 1, -1, vbBinaryCompare)
    
    
    ConvertJsonSymbol = strJSON
End Function

Public Function RestoreJsonSymbol(ByVal strJSON As String) As String

    strJSON = Replace(strJSON, "%0D", vbCr, 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "%0A", vbLf, 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "%26", "&", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "%20", " ", 1, -1, vbBinaryCompare)
    strJSON = Replace(strJSON, "%39", "'", 1, -1, vbBinaryCompare)
    ...
    For Each v In dic.keys
        ...
        If Not IsEmpty(v) Then
            ...
            Select Case ValType
                ...
                Case Else
                    Call SetOtherCaseValue(SB, ConvertJsonSymbol(dic.Item(v)))
            End Select
    ...
    For i = 1 To Col.Count
        ...
        Select Case ValType
            ...
            Case Else
               ...
               Call SetOtherCaseValue(SB, ConvertJsonSymbol(Col.Item(i)))
...
'Public Function ConvertChar(ByVal s As String) As String
'
'    s = VBA.Replace(s, """", "|*shuangyinhao*|", 1, -1, vbBinaryCompare)
'    s = VBA.Replace(s, "'", "|*dangyinhao*|", 1, -1, vbBinaryCompare)
'    s = VBA.Replace(s, ":", "|*maohao*|", 1, -1, vbBinaryCompare)
'    ConvertChar = s
'End Function
'
'Public Function RestoreChar(ByVal s As String) As String
'    s = VBA.Replace(s, "|*shuangyinhao*|", """", 1, -1, vbBinaryCompare)
'    s = VBA.Replace(s, "|*dangyinhao*|", "'", 1, -1, vbBinaryCompare)
'    s = VBA.Replace(s, "|*maohao*|", ":", 1, -1, vbBinaryCompare)
'    RestoreChar = s
