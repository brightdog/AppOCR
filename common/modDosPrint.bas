...
104         strResult = objShell.Run("c:\windows\system32\cmd.exe /q /c " & strCommand & " > """ & App.Path & "\tmp.tmp""", 0, WaitOnReturn)
...
106         iFreeFileNum = VBA.FreeFile()
    
            
    
108         Open App.Path & "\tmp.tmp" For Input As #iFreeFileNum
...
124         VBA.Shell strCommand, vbNormalNoFocus
...
DosPrint_Err:
        ...
        WriteLog Err.Number & ":" & Err.Description & vbCrLf & "in modDosPrint.DosPrint " & "at line " & Erl, True
        Resume Next
        '</EhFooter>
End Function
